...
def update_db(json_, k):
    ...
    for i in b:
        ...
        #print(time)
        query = 'update route_stop_bridge set eta_time = \''+str(time)+'\' where (key = \''+k[counter]+'\') and (eta_time > \''+str(time)+'\' or (COALESCE(eta_time,\'a\') = \'a\'));'
        counter += 1
        #print(query)
        cur.execute(query)
    conn.commit()
...
def get_points(route_id):
    ...
    try:
        ...
        exit()
    cur = conn.cursor()
    query = 'select latitude, longitude, key from route_stop_bridge B inner join stops S on B.stop_id = S. stop_id where (B.route_id = \''+str(route_id)+'\') and (S.stop_name != \'Way Point\') order by B.rank;'
    cur.execute(query)
    output = cur.fetchall()
    #print(output)
    return output
